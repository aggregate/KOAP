...
int main(int argc, char** argv)
{
	...
	printf("Size of kernel string: %d\n", (int)ksize);
...
/* set args for second kernel */
		errCode =
	clSetKernelArg(kernel_1, 0, sizeof(cl_mem), (void*)&cl_a);
	if (!checkError(errCode, "clSetKernelArg(0):")){return 0;}

		errCode =
	clSetKernelArg(kernel_1, 1, sizeof(cl_mem), (void*)&cl_b);
	if (!checkError(errCode, "clSetKernelArg(1):")){return 0;}

		errCode =
	clSetKernelArg(kernel_1, 2, sizeof(cl_mem), (void*)&cl_out);
	if (!checkError(errCode, "clSetKernelArg(2):")){return 0;}
...
*/
		...
		{
			errCode =
			clEnqueueNDRangeKernel(comQ, kernel_1, 1, NULL, (const size_t *)&gws,
